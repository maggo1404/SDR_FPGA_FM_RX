...
for sample in iq_stream:
    dc_sum += sample - fifo[-1]
    dc_blocked_sample = sample - (dc_sum / fifo_size)
    fifo[:-1] = fifo[1:]
    fifo[-1] = sample
    dc_blocked_stream.append(dc_blocked_sample)

dc_blocked_stream = np.array(dc_blocked_stream)
...
duration = len(dc_blocked_stream) / sample_rate
time = np.linspace(0, duration, len(dc_blocked_stream))
frequency = 629000
sinusoid = np.exp(-1j * 2 * np.pi * frequency * time)  # Negatives Vorzeichen hinzugefügt

# 4. IQ-Stream und Sinus multiplizieren (mischen)
mixed_stream = dc_blocked_stream * sinusoid
